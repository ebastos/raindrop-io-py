...
refresh_manage:
    @uv remove manage --group dev
    @uv add git+https://github.com/PBorocz/manage --group dev
...
docs *args:
    ...
    @uv run sphinx-build -v -W -b html "docs" "docs/_build" {{args}}
...
fawltydeps *args:
    @uv run time fawltydeps --detailed {{args}}
